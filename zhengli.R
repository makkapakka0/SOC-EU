...
for(i in tree){
  for (j in try){
    rf<-randomForest(OC~ele+tp+tem+NDVI+lc+pm,
                     ntree=i,
                     mtry=j,
                     data=df)
    obbmse[i/10,j]<-rf[["mse"]][length(rf[["mse"]])]
  }
}

write_xlsx(obbmse,'D:\\2paper\\working\\pamameters.csv')

obbmse<-read.csv(paste0('D:\\2paper\\working\\para.csv'))
obbmse<-t(obbmse)

heatmap(as.matrix(obbmse[, c('mtry1','mtry2','mtry3')]),
        scale='none',
        Rowv = obbmse$ntree, 
        Colv = NA)+
  coord_flip()
